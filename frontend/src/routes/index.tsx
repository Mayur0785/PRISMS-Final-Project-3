  ...
  return (
    <div className="min-h-screen bg-[#fbfbfa] text-slate-900 flex flex-col font-sans selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1 w-full space-y-6">
        ...
        {/* TAB 4: MY CROPS */}
        {/* ========================================================================= */}
        {/* TAB 4: MY CROPS & HARVEST MANAGEMENT */}
        {activeTab === "crops" && (
          <div className="px-6 pb-6 pt-3 md:px-8 md:pb-8 md:pt-4 relative z-10 flex flex-col gap-6 flex-1">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="text-[28px] md:text-[34px] font-bold text-on-surface tracking-tight">
                    {lang === "mr" ? "माझी सक्रिय पिके व साठा" : "My Active Crop Batches"}
                  </h2>
                  {demoMode && (
                    <span className="px-2.5 py-0.5 bg-alert-terracotta/20 text-alert-terracotta text-[11px] font-bold rounded-full border border-alert-terracotta/30">
                      DEMO MODE
                    </span>
                  )}
                </div>
                <p className="text-[15px] text-on-surface-variant">
                  {lang === "mr"
                    ? "पिकांचे वजन, अंदाजित बाजार मूल्य आणि काढणी व्यवस्थापन पहा."
                    : "Track harvest timelines, batch quantities and real-time mandi valuations."}
                </p>
              </div>
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3.5 py-2.5 bg-surface-container-high border border-outline-variant text-on-surface rounded-lg font-bold text-[12px] hover:bg-surface transition-colors"
                >
                  {demoMode ? (lang === "mr" ? "डेमो बंद करा" : "Exit Demo Mode") : (lang === "mr" ? "नमुना डेमो पहा" : "Load Demo Mode")}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-5 py-2.5 bg-primary text-on-primary rounded-lg font-bold text-[14px] flex items-center gap-2 hover:bg-primary-container transition-colors shadow-sm active:scale-95"
                >
                  <span className="material-symbols-outlined text-[18px]">add_circle</span>
                  {lang === "mr" ? "नवीन पीक नोंदवा" : "Add Harvest Batch"}
                </button>
              </div>
            </div>
            ...
            {!currentUser && !demoMode ? (
              <div className="bg-surface-container-high border border-outline-variant rounded-2xl p-10 text-center max-w-xl mx-auto my-6 shadow-sm">
                <div className="w-16 h-16 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mx-auto mb-4 font-bold text-3xl">
                  🌾
                </div>
                <h3 className="text-[22px] font-extrabold text-on-surface">
                  {lang === "mr" ? "आपली पिके ट्रॅक करण्यासाठी लॉग इन करा" : "Sign in to track your harvest batches"}
                </h3>
                <p className="text-[14px] text-on-surface-variant mt-2 mb-6 font-medium">
                  {lang === "mr"
                    ? "तुमच्या पिकांची खरी निव्वळ उत्पन्न मोजणी आणि बाजारपेठ वेळेचे नियोजन करण्यासाठी खाते तयार करा."
                    : "Create an account or sign in to save your personal crop batches, compute net realization, and optimize sales timing."}
                </p>
                <div className="flex justify-center gap-3">
                  <button
                    type="button"
                    onClick={() => setAuthModalOpen(true)}
                    className="px-6 py-3 bg-primary text-on-primary rounded-xl font-bold text-[14px] shadow hover:bg-primary-container transition-all active:scale-95"
                  >
                    {lang === "mr" ? "लॉग इन करा / नोंदणी करा" : "Sign In / Register"}
                  </button>
                  <button
                    type="button"
                    onClick={() => setDemoMode(true)}
                    className="px-4 py-3 border border-outline-variant bg-surface text-on-surface rounded-xl font-bold text-[13px] hover:bg-surface-container transition-colors"
                  >
                    {lang === "mr" ? "नमुना डेमो पहा" : "Load Demo Data"}
                  </button>
                </div>
              </div>
            ) : activeBatches.length === 0 ? (
              /* Genuine New User Empty State (0 Batches) - Compact Layout */
              <div className="bg-surface-container-high border border-dashed border-outline-variant rounded-2xl p-5 text-center max-w-lg mx-auto my-2 shadow-xs">
                <div className="flex items-center justify-center gap-2.5 mb-2">
                  <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-lg font-extrabold shrink-0">
                    🌱
                  </div>
                  <h3 className="text-[16px] font-bold text-on-surface">
                    {lang === "mr" ? "अद्याप कोणतेही सक्रिय पीक नाही" : "No active crop batches yet"}
                  </h3>
                </div>
                <p className="text-[12px] text-on-surface-variant mb-3 font-medium leading-relaxed max-w-md mx-auto">
                  {lang === "mr"
                    ? "तुमचे पहिले पीक जोडण्यासाठी + नवीन पीक नोंदवा बटणावर क्लिक करा आणि खरा नफा पहा."
                    : "Add your first harvest batch to start tracking net realization, APMC cess, and transport freight."}
                </p>
                <button
                  type="button"
                  onClick={() => setNewBatchModalOpen(true)}
                  className="px-4 py-2 bg-primary text-on-primary rounded-xl font-bold text-[13px] shadow hover:bg-primary-container transition-all active:scale-95 inline-flex items-center gap-1.5"
                >
                  + {lang === "mr" ? "नवीन पीक नोंदवा" : "Add Harvest Batch"}
                </button>
              </div>
            ) : (
              /* User-Owned Batch Cards Grid */
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {activeBatches.map((batch) => (
                  <div
                    key={batch._id}
                    className="bg-surface-container-high border border-outline-variant rounded-xl p-5 tactile-hover flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-[24px]">
                          {CROP_EMOJI[batch.cropName] || "🌱"}
                        </span>
                        <span
                          className={`px-2.5 py-0.5 font-bold rounded text-[11px] ${
                            batch.status === "Peak Price"
                              ? "bg-success-sage/20 text-success-sage"
                              : batch.status === "Holding (Wait)"
                              ? "bg-secondary-container text-on-secondary-container"
                              : "bg-surface-variant text-on-surface-variant"
                          }`}
                        >
                          {batch.status || "Standard"}
                        </span>
                      </div>
                      <h3 className="text-[18px] font-bold text-on-surface">{batch.cropName}</h3>
                      <p className="text-[12px] text-on-surface-variant mt-0.5 font-medium">
                        Batch #{batch._id.slice(-4)} • {batch.quantityKg.toLocaleString("en-IN")} Kg ({batch.grade})
                      </p>
                    </div>
                    <div className="mt-4 pt-3 border-t border-outline-variant/50 flex justify-between items-end">
                      <div>
                        <span className="text-[10px] font-bold uppercase text-on-surface-variant">
                          {lang === "mr" ? "अंदाजित उत्पन्न" : "Est. Realization"}
                        </span>
                        <p className="text-[20px] font-extrabold text-primary">
                          ₹{(batch.estimatedRealization || 0).toLocaleString("en-IN")}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <button
                          type="button"
                          onClick={() => {
                            setCropId(batch.cropName.toLowerCase().includes("onion") ? "onion_1" : "wheat_1");
                            setQty(String(batch.quantityKg));
                            setActiveTab("analytics");
                          }}
                          className="text-primary font-bold text-[12px] hover:underline"
                        >
                          {lang === "mr" ? "विश्लेषण पहा →" : "View Analytics →"}
                        </button>
                        <button
                          type="button"
                          onClick={async () => {
                            if (demoMode) {
                              alert("Demo mode batches cannot be deleted.");
                              return;
                            }
                            try {
                              await deleteUserCrop(batch._id);
                              await queryClient.invalidateQueries({ queryKey: ["userCrops"] });
                              setSavedToastMsg(lang === "mr" ? "पीक विक्री पूर्ण झाली!" : "Crop batch sold & updated!");
                              setSavedToast(true);
                              setTimeout(() => setSavedToast(false), 3000);
                            } catch (err: any) {
                              alert(err?.response?.data?.error?.message || "Failed to delete crop");
                            }
                          }}
                          className="text-[11px] font-bold text-alert-terracotta hover:underline"
                        >
                          {lang === "mr" ? "विक्री पूर्ण झाली" : "Mark Sold"}
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            {newBatchModalOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                <div className="bg-surface rounded-2xl border border-outline-variant max-w-md w-full p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-[18px] font-bold text-on-surface">
                      {lang === "mr" ? "नवीन पीक नोंदवा" : "Register Harvest Batch"}
                    </h3>
                    <button
                      type="button"
                  ...
                  <form
                    onSubmit={async (e) => {
                      ...
                      try {
                        if (demoMode) {
                          setSavedToastMsg(lang === "mr" ? "डेमो पीक साठा जोडला!" : "Demo crop batch added!");
                        } else {
                          await createUserCrop({
                            cropName: newCropName,
                            quantityKg: Number(newCropQty) || 1000,
                            grade: "Grade 1",
                            targetMandi: "Vashi APMC",
                          });
                          await queryClient.invalidateQueries({ queryKey: ["userCrops"] });
                          setSavedToastMsg(lang === "mr" ? "नवीन पीक साठा डेटाबेसमध्ये जतन केला!" : "Crop batch saved to database!");
                        }
                        setSavedToast(true);
                        setNewBatchModalOpen(false);
                        setTimeout(() => setSavedToast(false), 3000);
                      } catch (err: any) {
                        alert(err?.response?.data?.error?.message || "Failed to save crop batch");
                      }
                    }}
                    className="space-y-4"
                  >
                    <div>
                      <label className="block text-[12px] font-bold text-on-surface-variant mb-1">
                        {lang === "mr" ? "पीक निवडा" : "Select Crop"}
                      </label>
                      <select
                        value={newCropName}
                        onChange={(e) => setNewCropName(e.target.value)}
                        className="w-full bg-surface-container-low border border-outline-variant rounded-lg p-2.5 text-[14px] font-bold text-on-surface focus:ring-2 focus:ring-primary outline-none"
                      >
                        <option value="Red Onion">🧅 {lang === "mr" ? "लाल कांदा (नाशिक)" : "Red Onion (Nashik)"}</option>
                        <option value="Wheat">🌾 {lang === "mr" ? "शरबती गहू" : "Sharbati Wheat"}</option>
                        <option value="Soybeans">🌱 {lang === "mr" ? "पिवळा सोयाबीन" : "Yellow Soybeans"}</option>
                        <option value="Tomato">🍅 {lang === "mr" ? "टोमॅटो (हायब्रिड)" : "Tomato (Hybrid)"}</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-[12px] font-bold text-on-surface-variant mb-1">
                        {lang === "mr" ? "वजन (किलो मध्ये)" : "Quantity (in Kg)"}
                      </label>
                      <input
                        type="number"
                        value={newCropQty}
                    ...
                    <button
                      type="submit"
                      className="w-full bg-primary text-on-primary py-3 rounded-lg font-bold text-[14px] hover:bg-primary-container transition-colors shadow-sm active:scale-95 mt-2"
                    >
                      {lang === "mr" ? "पीक साठ्यात जोडा" : "Add to Active Portfolio"}
                    </button>
                  </form>
                </div>
              </div>
            ...
            {/* Trade Lots Management Component */}
