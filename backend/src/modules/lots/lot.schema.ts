...
export const createLotSchema = z.object({
  body: z.object({
    ...
    quantityQtl: z.number().positive('Quantity must be greater than 0'),
    qualityScore: z.number().min(0).max(100).optional(),
    origin: z.string().optional(),
    district: z.string().optional(),
    targetMarket: z.string().optional(),
    expectedPricePerQtl: z.number().positive('Expected price must be positive'),
    minimumAcceptablePrice: z.number().positive('Minimum price must be positive').optional(),
